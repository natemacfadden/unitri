...
/*
   ...
   This file: exact arbitrary-precision (GMP / mpz_t) variant of na-query.c.
   A single run yields the exact integer count, so no CRT reconstruction is
   needed.
*/
...
// values are exact mpz_t integers; H[v] points into the mpz_t page memH[v],
...
mpz_t query_value;        // exact count at the query profile
mpz_t recurrence_sum;     // reusable accumulator for the per-cell recurrence
...
int main( int argc, char *argv[] ){
  ...
  for (twice_area=0; twice_area<=m; twice_area++) {
    mem = n2pow[m-3]*sizeof(mpz_t ***);
    alloc_pointers += mem;
    if (!only_mem) H[twice_area] = (mpz_t ****)malloc( mem );

    for (int coord=0; coord<n2pow[m-3]; coord++) {
      mem = n2*sizeof(mpz_t **);
      alloc_pointers += mem;
      if( ! only_mem ) H[twice_area][coord] = (mpz_t ***)malloc( mem );

      for (height_m2=0; height_m2<n2; height_m2++) {
        mem = n2*sizeof(mpz_t *);
        alloc_pointers += mem;
        if (!only_mem) H[twice_area][coord][height_m2] =
            (mpz_t **)malloc( mem );
      }
    }
  }
  ...
  if (!only_mem) {
    ...
    mpz_set_ui(H[0][0][0][0][0], lower_profile_is_zero() ? 1 : 0);
  }
  ...
  for (twice_area=1; twice_area<=2*m*n; twice_area++) {
    ...
    mem_page_size = allo;
     ...
     mem = mem_page_size*sizeof(mpz_t);
            ...
            mpz_set_ui(recurrence_sum, 0);
             ...
             while(1){      /* loop over one-step subshapes */
                 ...
                 if (need_full_table
                     ...
                     if (inclusion_sign[m] > 0)
                       mpz_add(recurrence_sum, recurrence_sum,
                       ...
                       mpz_sub(recurrence_sum, recurrence_sum,
                           H[twice_area - removed_twice_area[m]]
                              [shape_index + subshape_index_delta[m-3]]
                              [height_options_m2[subshape_code[m-2]]]
                              [height_options_m1[subshape_code[m-1]]]
             ...
             recurrence_value_with_lower_base(
...
gmp_printf("] = %Zd\n",recurrence_sum);
     ...
     if (!query_enabled && twice_area%(2*m) == 0) {
      ...
      gmp_printf("(* f(%d,%d) = *) %Zd",m,k,H[twice_area][k*shape_index][k][k][k]);
      if(k<n) puts(",");
      else puts("};");
     }
